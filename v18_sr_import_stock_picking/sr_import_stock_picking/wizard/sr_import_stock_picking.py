...
class srImportStockPicking(models.TransientModel):
    ...
    def _create_move(self, picking, row):
        ...
        if self.import_option == 'csv':
            ...
            elif isinstance(raw_date, (int, float)):  # Excel numeric date
                scheduled_date = from_excel(raw_date)
            elif isinstance(raw_date, str):
                try:
                    scheduled_date = datetime.strptime(raw_date, '%d/%m/%Y')
                except ValueError:
                    raise UserError(_("Invalid date format '%s'. Expected format: 'DD/MM/YYYY'") % raw_date)
            else:
                scheduled_date = datetime.now()
