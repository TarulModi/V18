...
class SaleOrder(models.Model):
    ...
    @api.depends('is_backorder', 'demand_qty', 'order_line')
    def _get_backorder_details(self):
        for record in self:
            backorder_id = self.env['stock.picking'].search([('backorder_id','!=',False),('origin','=',record.name),('state','in',['confirmed','assigned'])])
            if backorder_id:
