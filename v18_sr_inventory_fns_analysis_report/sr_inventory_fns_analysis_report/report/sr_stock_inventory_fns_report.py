...
class srReportStockInventoryFNS(models.AbstractModel):
    ...
    _description = "Inventory FNS Analysis Report"
    ...
    def _get_report_values(self, docids, data=None):
        ...
        for product in products:
            ...
            opening_balance = sum(move.quantity for move in
                                  purchase_moves.filtered(lambda rec: rec.date.date() < docs.start_date)) - sum(
                move.quantity for move in sales_moves.filtered(lambda rec: rec.date.date() < docs.start_date))
            ...
            for day in range(0, total_days):
                ...
                issue_qty = sum(move.quantity for move in today_moves.filtered(
