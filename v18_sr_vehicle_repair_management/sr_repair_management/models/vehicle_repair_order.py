...
class VehicleRepairOrder(models.Model):
    _name = "vehicle.repair.order"
    _description = "Vehicle Repair Order"
    ...
    repair_details_ids = fields.One2many("repair.details", "repair_id", copy=False, tracking=True)

    # Diagnosis Details
    diagnosis_id = fields.Many2one('repair.diagnosis', copy=False, tracking=True)
    repair_estimate_ids = fields.One2many(related="diagnosis_id.repair_estimate_ids")
    total = fields.Float(related="diagnosis_id.total")
    est_delivery_date = fields.Date(related="diagnosis_id.est_delivery_date")
    sale_order_ids = fields.Many2many(related="diagnosis_id.sale_order_ids")
    repair_work_order_ids = fields.Many2many(related="diagnosis_id.repair_work_order_ids")
    sale_order_count = fields.Integer(related="diagnosis_id.sale_order_count")
    work_order_count = fields.Integer(related="diagnosis_id.work_order_count")
    ...
    def _compute_diagnosis_count(self):
        for record in self:
            seach_count = self.env['repair.diagnosis'].search_count([('id', '=', self.diagnosis_id.id)])
            record.diagnosis_count = seach_count

    @api.model_create_multi
    def create(self, vals_list):
        for vals in vals_list:
            vals['name'] = self.env['ir.sequence'].next_by_code('vehicle.repair.order') or "New"
    ...
    def create_diagnosis(self):
        if not self.repair_details_ids:
            raise ValidationError(_("Please add repair details."))
        self.ensure_one()
        ...
        diagnosis_vals = {
            ...
            'checklist_ids': [
                (0, 0, {
                    ...
                    'repair_details_id': checklist.id,
                    'name': checklist.name,
                    'description': checklist.description,
                    'attachment_ids': [(6, 0, checklist.attachment_ids.ids)],
                }) for checklist in self.repair_details_ids
            ]
        }

        diagnosis = self.env['repair.diagnosis'].create(diagnosis_vals)
        if diagnosis:
            self.write({
                'state': 'in_diagnosis',
                'diagnosis_id' : diagnosis.id,
            })
            if diagnosis.checklist_ids:
                for checklist in diagnosis.checklist_ids:
                    if checklist.repair_details_id:
                        checklist.repair_details_id.write({
                            'checklist_id' : checklist.id,
                            'diagnosis_id' : checklist.diagnosis_id.id,
                            'state': 'in_diagnosis',
                        })
            else:
                self.repair_details_ids.write({
                    'state': 'in_diagnosis',
                })
